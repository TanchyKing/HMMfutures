...
# 获取数据
data = pd.read_excel('lppp HMM.xlsx')
dataRATE = pd.read_excel('PP HMM.xlsx')
dcol = data.columns
# 日期列表
...
while (k < 5):
    ...
    for i in range(days - 1):
        # 每天交易的手数
        shares[i + 1] = (money[i] * store) // (0.5 * close[i])
        money[i + 1] = money[i]
        stop = 0
        # 看涨
        if long_sig[i] and not short_sig[i]:
            trademoney = 5 * shares[i + 1] * openi[i + 1]
            money[i + 1] = money[i] - trademoney
            if high[i + 1] >= 1.04 * openi[i + 1]:
                trademoney = 5 * shares[i + 1] * 1.04 * openi[i + 1]
            elif low[i + 1] <= 0.98 * openi[i + 1]:
                trademoney = 5 * shares[i + 1] * 0.98 * openi[i + 1]
            else:
                trademoney = 5 * shares[i + 1] * close[i + 1]
            money[i + 1] += trademoney
            money[i + 1] -= 5 * shares[i + 1]
        # 看跌
        if not long_sig[i] and short_sig[i]:
            trademoney = 5 * shares[i + 1] * openi[i + 1]
            money[i + 1] = money[i] + trademoney
            if low[i + 1] <= 0.96 * openi[i + 1]:
                trademoney = 5 * shares[i + 1] * 0.96 * openi[i + 1]
            elif high[i + 1] >= 1.02 * openi[i + 1]:
                trademoney = 5 * shares[i + 1] * 1.02 * openi[i + 1]
            else:
                trademoney = 5 * shares[i + 1] * close[i + 1]
            money[i + 1] -= trademoney
            money[i + 1] -= 5 * shares[i + 1]
