...
# 获取数据
data = pd.read_excel('棕榈油 hmm.xlsx')
dataRATE = pd.read_excel('棕榈油.xlsx')
dcol = data.columns
# 日期列表
    ...
    for i in range(days - 1):
        ...
        if long_sig[i] and not short_sig[i]:
            ...
            if high[i + 1] >= 1.008 * openi[i + 1]:
                trademoney = 10 * shares[i + 1] * 1.008 * openi[i + 1]
            elif low[i + 1] <= 0.995 * openi[i + 1]:
                trademoney = 10 * shares[i + 1] * 0.995 * openi[i + 1]
            else:
        ...
        if not long_sig[i] and short_sig[i]:
            ...
            if low[i + 1] <= 0.992 * openi[i + 1]:
                trademoney = 10 * shares[i + 1] * 0.992 * openi[i + 1]
            elif high[i + 1] >= 1.005 * openi[i + 1]:
                trademoney = 10 * shares[i + 1] * 1.005 * openi[i + 1]
            else:
                trademoney = 10 * shares[i + 1] * close[i + 1]
            money[i + 1] -= trademoney
            money[i + 1] -= 10 * shares[i + 1]
