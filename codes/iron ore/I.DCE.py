...
while (k < 5):
    ...
    for i in range(days - 1):
        # 每天交易的手数
        shares[i + 1] = (money[i] * store) // (12 * close[i])
        money[i + 1] = money[i]
        stop = 0
        # 看涨
        if long_sig[i] and not short_sig[i]:
            trademoney = 100 * shares[i + 1] * openi[i + 1]
            money[i + 1] = money[i] - trademoney
            if high[i + 1] >= 1.05 * openi[i + 1]:
                trademoney = 100 * shares[i + 1] * 1.05 * openi[i + 1]
            elif low[i + 1] <= 0.975 * openi[i + 1]:
                trademoney = 100 * shares[i + 1] * 0.975 * openi[i + 1]
            else:
                trademoney = 100 * shares[i + 1] * close[i + 1]
            money[i + 1] += trademoney
            money[i + 1] -= 100 * shares[i + 1]
        # 看跌
        if not long_sig[i] and short_sig[i]:
            trademoney = 100 * shares[i + 1] * openi[i + 1]
            money[i + 1] = money[i] + trademoney
            if low[i + 1] <= 0.95 * openi[i + 1]:
                trademoney = 100 * shares[i + 1] * 0.95 * openi[i + 1]
            elif high[i + 1] >= 1.025 * openi[i + 1]:
                trademoney = 100 * shares[i + 1] * 1.025 * openi[i + 1]
            else:
                trademoney = 100 * shares[i + 1] * close[i + 1]
            money[i + 1] -= trademoney
            money[i + 1] -= 100 * shares[i + 1]
