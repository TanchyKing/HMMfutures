...
# 获取数据
data = pd.read_excel('tongp HMM.xlsx')
dataRATE = pd.read_excel('tong HMM.xlsx')
dcol = data.columns
# 日期列表
...
while (k < 5):
    ...
    for i in range(days - 1):
        ...
        if long_sig[i] and not short_sig[i]:
            ...
            if high[i + 1] >= 1.03 * openi[i + 1]:
                trademoney = 5 * shares[i + 1] * 1.03 * openi[i + 1]
            elif low[i + 1] <= 0.985 * openi[i + 1]:
                trademoney = 5 * shares[i + 1] * 0.985 * openi[i + 1]
            else:
        ...
        if not long_sig[i] and short_sig[i]:
            trademoney = 5 * shares[i + 1] * openi[i + 1]
            money[i + 1] = money[i] + trademoney
            if low[i + 1] <= 0.97 * openi[i + 1]:
                trademoney = 5 * shares[i + 1] * 0.97 * openi[i + 1]
            elif high[i + 1] >= 1.015 * openi[i + 1]:
                trademoney = 5 * shares[i + 1] * 1.015 * openi[i + 1]
            else:
                trademoney = 5 * shares[i + 1] * close[i + 1]
            money[i + 1] -= trademoney
            money[i + 1] -= 5 * shares[i + 1]
